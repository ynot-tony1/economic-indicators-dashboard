...
## Migration to Google Cloud (2026-09-24)

Moved the app onto GCP (project `marketpersonalities`, region `europe-west2`), for interview prep. Everything is scripted in `gcp/deploy.sh` (stages: `infra`, `migrate`, `build`, `deploy`, `schedule`, `run-job`, `cost-guard`).

- **Web → Cloud Run service `mp-web`.** `output: "standalone"` in `next.config.ts` + multi-stage `web/Dockerfile` (242 MB image, non-root). The build needs the DB (featured pages prerender from live data), so the connection string is passed as a **BuildKit secret mount**, never an ARG/ENV — verified with `docker history`/`inspect` that the password is in no layer. Live: https://mp-web-853132539656.europe-west2.run.app
- **Pipeline → Cloud Run Job `mp-pipeline`** (`scraper/Dockerfile`, `run_pipeline.sh`: scrape → insights → BigQuery), triggered by **Cloud Scheduler `mp-nightly`** at `0 0 * * *` in `Europe/London` — Scheduler is time-zone aware, so the GitHub Actions dual-cron DST workaround isn't needed here.
- **Database → Cloud SQL Postgres 16 (`db-f1-micro`)**. `gcp/migrate_cockroach_to_cloudsql.py` applied the schema, truncated the seed rows (they'd have fresh UUIDs clashing with the source), copied all 5 tables in FK order (SELECT from CockroachDB — its `COPY TO` support is patchier — then `COPY FROM` into Postgres), and verified counts: 179 / 13,388 / 17,434 / 841 / 157, all matching. Local tools reach Cloud SQL through the **Cloud SQL Auth Proxy** (`--gcloud-auth`); Cloud Run uses the built-in connector's unix socket.
- **BigQuery** (`scraper/export_bigquery.py`): current-state tables full-refresh; `indicator_snapshots` partitioned by day on `scraped_at` and clustered by `indicator_id`; `country_traits_history` / `country_personas_history` get one date partition per run, written via `table$YYYYMMDD` + `WRITE_TRUNCATE` so a same-day rerun is idempotent.
- **IAM:** one service account per workload — `mp-web` (Cloud SQL client, secret accessor), `mp-pipeline` (+ BigQuery data editor/job user), `mp-scheduler` (may only invoke this one job), `mp-cost-guard` (Cloud SQL editor), `mp-pubsub-push` (may only invoke the cost guard). Secrets (`db-password`, `database-url`) in Secret Manager.
- **Cost:** the billing account from 2024 had been closed (old trial expired), so a new billing account was opened. Everything except Cloud SQL sits inside always-free limits; Cloud SQL is ~£7–8/month. Budget alert at **£15**, and because budgets only notify, `gcp/cost_guard/` is a real cap: budget → Pub/Sub topic `budget-alerts` → authenticated push → Cloud Run service `mp-cost-guard` → sets the instance's activation policy to `NEVER` once cost ≥ budget. Verified end to end with a dry-run message (`dry_run=true` attribute): first deliveries 403'd until the fresh `run.invoker` grant propagated, Pub/Sub retried, and the guard logged "OVER budget 16.00/15.00 — dry run, would stop market-personalities-db". Billing data lags hours, so the cap is approximate.
- **Gotchas hit and fixed:** (1) libpq `sslmode=verify-full` fails in a fresh container (no `~/.postgresql/root.crt`) — set `PGSSLROOTCERT` to the system CA bundle instead of weakening TLS; (2) the web build with `--network host` got intermittent DNS failures (`EAI_AGAIN`) — host networking is now only used in Cloud SQL mode, where it's needed to reach the local proxy; (3) `docker push` needs gcloud's credential helper on PATH — the script now adds it; (4) `.env.local` values are quoted — strip quotes when extracting.
