...
export function IndicatorCard({ countrySlug, row }: { countrySlug: string; row: LatestIndicatorRow }) {
  ...
  return (
    <Link href={`/${countrySlug}/${row.slug}`} className="group block">
      <Card
        className="h-full border-t-2 transition-all group-hover:-translate-y-0.5 group-hover:shadow-md [border-top-color:var(--accent-light)] dark:[border-top-color:var(--accent-dark)]"
